...
public class ActiveLinksPublisher extends AbstractLinkPublisher {
	...
	public ActiveLinksPublisher(SchedulerDef scheduler, Channel scrappingLinksChannel, Channel statusChangingLinksChannel) {
	...
	List<Link> findLinks(LinkDao linkDao) {
		return linkDao.findScrappingLinks(Grup.ACTIVE, retry, Props.getConfig().LIMITS.LINK_LIMIT_FETCHING_FROM_DB);
	}
